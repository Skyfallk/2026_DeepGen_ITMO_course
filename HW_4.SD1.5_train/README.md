...
### 1. Собрать датасет от 15 изображений одного персонажа (чем больше и разнообразнее, тем лучше будет качество)
...
Чтобы не было language drift (когда модель уже содержит знания в каком-то из токенов) мы используем токен sks он был выбран авторами статьи как один из наименее частотных в датасете.
